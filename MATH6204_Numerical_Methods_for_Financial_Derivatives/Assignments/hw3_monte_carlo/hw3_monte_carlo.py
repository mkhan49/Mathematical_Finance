...
def sample_paths_milstein(S0,r,q,sigma,T,dt):
    ''' Simulates the path for Stock price using Milstein discretization.
    ...
    for i in range(NRepl):
        for j in range(NSteps):
            St[i,j+1] = St[i,j] + (r-q) * St[i,j] * dt + sigma * St[i,j] * dt**0.5 * np.random.randn() + 0.5 * sigma**2 * St[i,j] * ((dt**0.5 * np.random.randn())**2 - dt)       
...
def monte_carlo_euler(S0,K,r,q,sigma,T,dt,OptionInd):
    ...
    return option_value, option_value_bsm, error

def monte_carlo_milstein(S0,K,r,q,sigma,T,dt,OptionInd):
    ''' Calculates Monte Carlo simulated  European call & put option value using Milstein discertization
    ...
    NSteps = int(T/dt)
    payoff = np.zeros((NRepl,1))
    paths = sample_paths_milstein(S0,r,q,sigma,T,dt)
    ...
    print ('%.3f      %.6f   %.6f   %.6f  Milstein' % (dt,option_value, option_value_bsm, error))
